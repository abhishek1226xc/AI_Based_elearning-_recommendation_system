...
export function registerChatRoutes(app: Express) {
  ...
  app.post("/api/chat", async (req, res) => {
    try {
      // Guard: if no Forge API configured, return a helpful message
      if (!ENV.forgeApiUrl || !ENV.forgeApiKey) {
        res.status(200).json({
          role: "assistant",
          content: "AI Chat is not available in local development mode. Configure BUILT_IN_FORGE_API_URL and BUILT_IN_FORGE_API_KEY in .env to enable it.",
        });
        return;
      }
