...
class OAuthService {
  constructor(private client: ReturnType<typeof axios.create>) {
    ...
    if (!ENV.oAuthServerUrl && !isTestEnv) {
      console.warn(
        "[OAuth] OAUTH_SERVER_URL is not configured; OAuth exchange endpoints may be unavailable."
      );
    }
  }
...
class SDKServer {
  ...
  async getUserInfo(accessToken: string): Promise<GetUserInfoResponse> {
    ...
    const userInfo = data as GetUserInfoResponse & UserInfoWithPlatforms;
  ...
  async getUserInfoWithJwt(
    ...
    const userInfo = data as GetUserInfoWithJwtResponse & UserInfoWithPlatforms;
    const loginMethod = this.deriveLoginMethod(
      userInfo.platforms,
      userInfo.platform ?? null
    );
